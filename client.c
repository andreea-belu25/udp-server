...
int main(int argc, char *argv[]) {
	...
	DIE(rc < 0, "setsockopt");
	rc = setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, &enable, sizeof(int)); // nagle
	DIE(rc < 0, "nagle");
	rc = connect(fd, (struct sockaddr *)&serv_addr, sizeof(serv_addr));
	DIE(rc < 0, "connect");
	rc = send(fd, argv[1], strlen(argv[1]), 0); // send id to the server
	DIE(rc < 0, "send");
	...
	while (1) {
		...
		if (fds[1].revents & POLLIN) {
			...
			DIE(rc < 0, "recv");
			if (message.type == TCP_QUIT) // server died
				exit(0);
