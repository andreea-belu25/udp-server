...
int receive_message(int file_descriptor, void *buffer) {
	short len;
	receive(file_descriptor, &len, sizeof(short)); // get the length of the message first
	return receive(file_descriptor, buffer, len); // get the message afterwards
}
...
void send_message_server(int file_descriptor, message_server message) {
	...
	if (message.type != TCP_QUIT) {
		...
		if (message.type == TCP_STRING)
			len = len + strlen(message.data) + 1; // + 1 because of null terminator
	}

	send_info(file_descriptor, &len, sizeof(short)); // send length first
	send_info(file_descriptor, (void *)&message, len); // send message afterwards
}

void send_message_client(int file_descriptor, message_client message) {
	short len = 1;
	if (message.type != CLIENT_QUIT)
		len = len + strlen(message.topic) + 1; // + 1 because of null terminator
